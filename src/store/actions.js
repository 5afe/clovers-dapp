...
export default {
  ...
  newSymFound({ dispatch }) {
    ...
    setTimeout(() => {
      dispatch('selfDestructMsg', {
        type: 'success',
        msg: 'New symmetrical clover found!',
        link: { name: 'Picks' },
      });
    }, 1000);
    // confetti:
  ...
  async cloverExists({ getters }, byteBoard) {
    ...
    return axios
      .get(getters.baseURL(`/clovers/${byteBoard}`))
      .then(() => true)
      .catch(() => false);
  },
  ...
  getCurrentUser({ getters, commit }, account) {
    ...
    return axios
      .get(getters.baseURL(`/users/${account.toLowerCase()}`))
  ...
  getClover({ commit, getters }, board) {
    ...
    return axios
      .get(getters.baseURL('/clovers/' + board))
      .then(({ data }) => {
  ...
  flagOrDeleteComment({ getters }, id) {
    return axios
      .delete(getters.baseURL(`/chats/${id}`), {
        headers: {
          Authorization: getters.authHeader,
        },
      })
  ...
  async signOut({ commit, dispatch }) {
    await await global.web3Connect.signOut();
    commit('SIGN_OUT');
    commit('UPDATE_WEB3', false);
    dispatch('selfDestructMsg', {
      type: 'success',
  ...
  async signIn({ state, dispatch, commit }, account) {
    if (!state.web3Enabled) {
      await global.web3Connect.signIn(); // open modal on button click
    } else {
  ...
  async divest({ state, dispatch }, { market, amount }) {
    ...
    if (marketContract === 'ClubTokenController') {
      ...
      balance = new BigNumber(balance);
      if (balance.lt(amount)) {
        throw new Error('balance too low: ' + balance + ' < ' + amount);
      }
  ...
  syncClover({ getters }, clover) {
    axios
      .get(getters.baseURL(`/clovers/sync/${clover.board}`))
      .catch((error) => {
        console.error(error);
      });
  },
  ...
  async buy({ getters, dispatch, state, commit }, clover) {
    ...
    if (cloverExists) {
      ...
      if (balanceOf.lt(currentPrice)) {
        value = await getLowestPrice(contracts, currentPrice.sub(balanceOf));
      }
  ...
  async makeCloverRFT({ state, dispatch }, { board, investmentInWei = 0 }) {
    ...
    // exists ?
    const exists = await contracts.Clovers.instance.methods
      .exists(board)
      .call();
    ...
    if (userBalance.lt(investmentInWei)) {
      value = await getLowestPrice(contracts, investmentInWei);
    }
    // go !
  ...
  getAllAlbums({ getters, commit, dispatch }, clover) {
    ...
    return axios
      .get(getters.baseURL('/albums/list/all'))
      .then((results) => {
        // console.log({results})
  ...
  getUserAlbums({ getters, state }) {
    ...
    return axios
      .get(getters.baseURL(`/users/${state.account}/albums`))
      .then(({ data }) => {
        return data;
      })
      .catch(console.error);
  ...
  async deleteAlbum({ dispatch, commit, getters }, albumId) {
    ...
    if (!(await dispatch('checkWeb3'))) throw new Error('Transaction Failed');
    return axios
      .delete(getters.baseURL('/albums/' + albumId), {
        headers: {
          Authorization: getters.authHeader,
        },
      })
  ...
  async updateAlbum({ getters, dispatch, commit, state }, album) {
    ...
    return axios
      .put(
        getters.baseURL('/albums/' + album.id),
  ...
  getAlbum({ getters, commit }, albumId) {
    ...
    return axios
      .get(getters.baseURL('/albums/' + albumId))
      .then(({ data }) => {
        if (!data) throw new Error('404');
...
async function getLowestPrice(
  ...
  if (!rr || !supply || !balance) {
    supply = await contracts.ClubToken.instance.methods.totalSupply().call();
    var virtualSupply = await contracts.ClubTokenController.instance.methods
      .virtualSupply()
      .call();

    supply = supply + virtualSupply;
    rr = await contracts.ClubTokenController.instance.methods
      .reserveRatio()
      .call();

    balance = await contracts.ClubTokenController.instance.methods
      .poolBalance()
      .call();
    var virtualBalance = await contracts.ClubTokenController.instance.methods
      .virtualBalance()
      .call();
  ...
  let resultOfSpend = await contracts.ClubTokenController.instance.methods
    .getBuy(currentPrice.toFixed())
    .call();
...
async function claimClover({ getters, keep, account, clover }) {
  ...
  if (keep) {
    ...
    if (balance.lt(mintPrice)) {
      ...
      value = await getLowestPrice(contracts, clubTokenToBuy);
    }
  }
  let signature;
  try {
